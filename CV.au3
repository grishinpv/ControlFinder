...
#AutoIt3Wrapper_Outfile=CF.exe
#AutoIt3Wrapper_Outfile_x64=CF_64.exe
...
;~ #AutoIt3Wrapper_Run_After=Utilities\ResHacker.exe -add %outx64%, %outx64%, Resources\CV.res,,,
...
Global Const $GUI_NAME = 'Control finder'
Global Const $GUI_VERSION = '1.1.2'
...
Func _ToogleState($h_Edit)

	;Local $iStyle = _WinAPI_GetWindowLongEx($h_Edit, $GWL_STYLE)
	;Local $iStyleNew = BitXOR($iStyle, $Style[19][0])			;$Style[29][0] VISIBLE 		$Style[19][0]	Disabled

	;MsgBox(0,"test", "_ToogleState Debug:" + @CRLF + "curStyle = 0x" + Hex($iStyle) + @CRLF + "curStyleStr = " + _GetStyleString($iStyle, 0, 0) + @CRLF + "newStyle = 0x" + Hex($iStyleNew) + @CRLF + "newStuleStr = " + _GetStyleString($iStyle, 0, 0))
	;MsgBox(0, "curStyle", Hex($iStyle,8))
	;MsgBox(0, "curStyleStr", _GetStyleString($iStyle, 0, 0))
	;MsgBox(0, "newStyle", Hex($iStyleNew,8))
	;MsgBox(0, "newStuleStr", _GetStyleString($iStyleNew, 0, 0))
	;Local CurrentStyleString = _GetStyleString($iStyle, 0, 0)
	;if (StringInStr(CurrentStyleString, "WS_DISABLED") or NOT (StringInStr(CurrentStyleString, "WS_VISIBLE")) Then
		;if (_WinAPI_SetWindowLong($h_Edit, $GWL_STYLE, $iStyleNew) == 0) Then
		;	MsgBox (16, "_ToogleState", "Operation Failed")
		;endif


	;EndIf

	Local $hWnd = HWnd($h_Edit)

	$fResult = False
	If _WinAPI_IsWindowEnabled ($hWnd) Then
		ControlDisable($hWnd, '', '')
	Else
		ControlEnable($hWnd, '', '')
	EndIf

EndFunc	;==>_ToogleState
...
Global $tPoint, $tRect
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Global $CtrlBtn, $WinBtn, $ComboCtrlActSelect, $ComboWinActSelect, $CtrlBtnSendMSG, $GetBufferBtn, $PutBufferBtn, $BuffMemo, $hBuffViwer
Global $WinSetBtn, $WinGetBtn, $CtrlSetBtn, $CtrlGetBtn, $CtrlToggleState
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		case $WinSetBtn
			if GUICtrlRead($Input[13])<>"" Then
				if not _WinAPI_SetWindowText(GUICtrlRead($Input[13]),GUICtrlRead($Input[3])) then MsgBox(64,"Error", "Cannot set window Title")
			Else
				MsgBox(64+262144,"Error", "Cannot set window Title. Handle is empty.",0,$hForm)
			EndIf
		Case $WinGetBtn
			if GUICtrlRead($Input[13])<>"" Then
				_SetData($Input[3], _WinAPI_GetWindowText(GUICtrlRead($Input[13])))
				MsgBox(64+262144,"CV requested Title",GUICtrlRead($Input[3]),0,$hForm)
			Else
				MsgBox(64+262144,"Error", "Cannot set window Title. Handle is empty.",0,$hForm)
			EndIf

		Case $CtrlSetBtn
			if GUICtrlRead($Input[29])<>"" Then
				_WM_SETTEXT(HWnd(GUICtrlRead($Input[29])), GUICtrlRead($Input[30]))
			Else
				MsgBox(64+262144,"Error", "Cannot set control Text. Handle is empty.",0,$hForm)
			EndIf

		Case $CtrlGetBtn
			if GUICtrlRead($Input[29])<>"" Then
				_SetData($Input[30],_WM_GETTEXT(HWnd(GUICtrlRead($Input[29]))))
				MsgBox(64,"Data from Control",GUICtrlRead($Input[30]),0,$hForm)
			Else
				MsgBox(64+262144,"Error", "Cannot set control Text. Handle is empty.",0,$hForm)
			EndIf

		Case $CtrlToggleState
			if GUICtrlRead($Input[29])<>"" Then
				_ToogleState(GUICtrlRead($Input[29]))
				;MsgBox(64,"Data from Control",GUICtrlRead($Input[30]),0,$hForm)
			Else
				MsgBox(64+262144,"Error", "Cannot set control Text. Handle is empty.",0,$hForm)
			EndIf
		...
		Case $GetBufferBtn
			MsgBox(64+262144,"Buffer read", _BuffGet(),0,$hForm)

		Case $PutBufferBtn
			_BuffPut("****" & @ScriptName & " clipboard set data****")
	...
	if @error Then
		Select
			Case @error=1
				$fRes = "<empty clip>"
			Case @error=2
				$fRes = "<ClipGet success, but contains non-text entry>"
			Case @error=3 or @error=4
				$fRes = "Access denied"
		EndSelect
		Return $fRes
	EndIf
...
Func _BuffPut($stext)
	$hData = __ClipBoard_SetData($stext)
EndFunc

Func __ClipBoard_SetData($vData, $iFormat = 1)
	...
	If Not _ClipBoard_Open($hForm) Then Return SetError(-5, 0, 0)
...
Func WM_DRAWCLIPBOARD($hWnd, $iMsg, $iwParam, $ilParam)
	Local $hClipOwner = _ClipBoard_GetOwner ()
	Local $fBuffData =_ClipBoard_GetData ()

	; try to get data from buffer by winapi, if return 0 then use wrapper function
;~ 	if not IsString($fBuffData) then
;~ 		consolewrite($fBuffData & @CRLF)
;~ 		$fBuffData = _BuffGet()
;~ 	EndIf
;~ 	Local $hClipTarget = _ClipBoard_GetOpenWindow ()
	; Display any text on clipboard
	MemoClear()
	MemoWrite("Clipboard owner handle....: " & $hClipOwner)
	MemoWrite("Clipboard owner proc name.: " & _ProcessGetName (WinGetProcess(HWnd($hClipOwner))))
;~ 	MemoWrite("Clipboard open window handle.: " & $hClipTarget)
;~ 	MemoWrite("Clipboard open window title..: " & WinGetTitle(HWnd($hClipTarget)))
	MemoWrite("Clipboard data:")
	MemoWrite(_ClipBoard_GetData ())
	MemoWrite()


	; Pass the message to the next viewer
	If $hBuffViwer <> 0 Then _SendMessage ($hBuffViwer, $WM_DRAWCLIPBOARD, $iwParam, $ilParam)
...
Func WM_CHANGECBCHAIN($hWnd, $iMsg, $iwParam, $ilParam)
	...
	If $iwParam = $hBuffViwer Then
		$hBuffViwer = $ilParam
		; Otherwise pass the message to the next viewer
	ElseIf $hBuffViwer <> 0 Then
		_SendMessage ($hBuffViwer, $WM_CHANGECBCHAIN, $iwParam, $ilParam, 0, "hwnd", "hwnd")
	EndIf
...
Func _WM_SETTEXT($h_Edit, $s_Text)
    ...
    _SendMessageA($h_Edit, $WM_SETTEXT, 0, DllStructGetPtr($struct_string))
EndFunc   ;==>_WM_SETTEXT
...
Func _WM_GETTEXT($h_Edit)
    ...
    _SendMessageA($h_Edit, $WM_GETTEXT, 4096, DllStructGetPtr($struct_string))
    If @error Then Return SetError(-1, -1, "")
...
Func MemoWrite($sMessage = "")
	GUICtrlSetData($BuffMemo, $sMessage & @CRLF, 1)
EndFunc   ;==>MemoWrite

Func MemoClear()
	_GUICtrlEdit_SetText($BuffMemo,"")
EndFunc
...
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	$Input[3] = GUICtrlCreateInput('', 80, 173, 200, 19)
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;~ Lacoil
;~
;~ 	$ComboWinActSelect = GUICtrlCreateCombo('', 265, 173, 50, 17, $CBS_DROPDOWNLIST)
;~ 	_GUICtrlComboBox_AddString(-1, 'Set')
;~ 	_GUICtrlComboBox_AddString(-1, 'Get')
;~ 	_GUICtrlComboBox_SetCurSel(-1, 0)
;~ 	$WinBtn = GUICtrlCreateButton("_SetWinText",320, 173, 70, 21)
	$WinSetBtn = GUICtrlCreateButton("Set",285, 173, 50, 21)
	$WinGetBtn = GUICtrlCreateButton("Get",340, 173, 50, 21)
...
;~	GUICtrlSetColor(-1, 0xAA0000)
	...
	$Input[15] = GUICtrlCreateInput('', 80, 373, 285, 19, $Style)
...
;~	GUICtrlSetColor(-1, 0x9999CC)
	...
	$Input[22] = GUICtrlCreateInput('', 168, 248, 220 - 108, 19, $Style)	; width - 108 for $CtrlToggleState
;~	GUICtrlSetColor(-1, 0xAA0000)
...
;~	GUICtrlSetColor(-1, 0xAA0000)
	...
	$Input[30] = GUICtrlCreateInput('', 80, 373, 200, 19)
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;~ Lacoil
;~
;~ 	$ComboCtrlActSelect = GUICtrlCreateCombo('', 265, 373, 50, 17, $CBS_DROPDOWNLIST)
;~ 	_GUICtrlComboBox_AddString(-1, 'Set')
;~ 	_GUICtrlComboBox_AddString(-1, 'Get')
;~ 	_GUICtrlComboBox_SetCurSel(-1, 0)
;~ 	;$CtrlBtn = GUICtrlCreateButton("Other func",320, 373, 70, 21)
;~ 	;$CtrlBtnSendMSG = GUICtrlCreateButton("_SendMsg",320, 348, 70, 21)
;~ 	$CtrlBtnSendMSG =GUICtrlCreateButton("_SendMsg",320, 373, 70, 21)

	$CtrlSetBtn = GUICtrlCreateButton("Set",285, 373, 50, 21)
	$CtrlGetBtn = GUICtrlCreateButton("Get",340, 373, 50, 21)

	$CtrlToggleState = GUICtrlCreateButton("Toggle State",285, 248, 102, 21)		; create near Style Label control

;~ New Tab with buffer
	GUICtrlCreateTabItem('Buffer')
	$GetBufferBtn =GUICtrlCreateButton("Get Buffer",Floor($_Crop[0] / 2-50), 373, 70, 21)
	$PutBufferBtn =GUICtrlCreateButton("Put Buffer",Floor($_Crop[0] / 2+50), 373, 70, 21)
	$BuffMemo = GUICtrlCreateEdit("", 36 + Floor((349 - $_Crop[0]) / 2), 171 + Floor((223 - $_Crop[1]) / 2), $_Crop[0], $_Crop[1]-30, BitOr($ES_WANTRETURN,$ES_READONLY,$WS_VSCROLL))
	GUICtrlSetFont(-1, 9, 400, 0, "Courier New")
	$hBuffViwer = _ClipBoard_SetViewer ($hForm)
...
;~	Next
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	_ClipBoard_ChangeChain ($hForm, $hBuffViwer)
